...
func _physics_process(delta: float) -> void:
	...
	if direction.x and abs(velocity.x) < behavior.speed:
		velocity.x += direction.x * behavior.speed * delta
	...
	if direction.y and abs(velocity.y) < behavior.speed:
		velocity.y += direction.y * behavior.speed * delta
		
	if not direction.y or sign(velocity.y) != direction.y:
		#adds friction when stopped or changing direction
		velocity.y /= behavior.drag
		if abs(velocity.y) < behavior.speed/100:
